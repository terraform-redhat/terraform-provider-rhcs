...
* Pre-release versions: These are beta versions of the provider, identified by a version number with a suffix introduced by a dash, such as 1.2.0-beta. While users can use these pre-release versions, it's important to note that doing so includes the responsibility of the user, as they may not be as stable or fully supported as official releases.
...
The Operator IAM roles will be created per cluster with the [rosa-classic](https://registry.terraform.io/modules/terraform-redhat/rosa-classic/rhcs/latest) module.
