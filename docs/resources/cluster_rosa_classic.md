...
- `autoscaling_enabled` (Boolean) Enable autoscaling for the initial worker pool. After the creation of the resource, it is not possible to update the attribute value.
- `availability_zones` (List of String) Availability zones.After the creation of the resource, it is not possible to update the attribute value.
...
- `compute_machine_type` (String) Identifies the machine type used by the default/initial worker nodes, for example `m5.xlarge`. Use the `rhcs_machine_types` data source to find the possible values.After the creation of the resource, it is not possible to update the attribute value.
- `default_mp_labels` (Map of String) This value is the default/initial machine pool labels. Format should be a comma-separated list of '{"key1"="value1", "key2"="value2"}'. After the creation of the resource, it is not possible to update the attribute value.
...
- `max_replicas` (Number) Maximum replicas of worker nodes in a machine pool. After the creation of the resource, it is not possible to update the attribute value.
- `min_replicas` (Number) Minimum replicas of worker nodes in a machine pool. After the creation of the resource, it is not possible to update the attribute value.
- `multi_az` (Boolean) Indicates if the cluster should be deployed to multiple availability zones. Default value is 'false'.After the creation of the resource, it is not possible to update the attribute value.
...
- `replicas` (Number) Number of worker/compute nodes to provision. Single zone clusters need at least 2 nodes, multizone clusters need at least 3 nodes. After the creation of the resource, it is not possible to update the attribute value.
...
- `worker_disk_size` (Number) Compute node root disk size, in GiB. After the creation of the resource, it is not possible to update the attribute value.
