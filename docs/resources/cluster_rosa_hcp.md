...
- `destroy_timeout` (Number) Maximum duration in minutes to wait for OCM to delete the cluster during destroy. Default value is 60 minutes. If the cluster still exists when the timeout expires, destroy fails and the resource remains in Terraform state so dependent STS resources (IAM roles, OIDC provider) are not removed while OCM uninstall may still be in progress.
