...
## FIPS usage and lifecycle

Set `fips = true` during cluster creation to request FIPS-enabled cryptographic modules for the cluster.

Lifecycle caveats:
- `fips` is immutable in this resource. Changing it after creation is rejected and requires creating a new cluster.
- Red Hat documents that FIPS must be enabled before first boot and cannot be enabled on an existing deployment. See [Support for FIPS cryptography](https://docs.redhat.com/en/documentation/openshift_container_platform/4.10/html/installing/installing-fips).

## Proxy and `no_proxy` behavior

When a proxy is configured on a zero-egress cluster, the OCM API appends platform-managed AWS endpoints (S3, STS, and ECR) to `no_proxy`. The provider filters these platform-injected entries from the Terraform state so that it stays in sync with your declared configuration.
...
- `auto_node` (Attributes) AutoNode configuration for ROSA HCP clusters. Currently only `enabled` mode is supported. (see [below for nested schema](#nestedatt--auto_node))
...
- `channel` (String) Y-stream specific channel for the cluster version (e.g., 'stable-4.16'). This parameter specifies the upgrade path for the cluster. Cannot be used together with 'channel_group'.
...
- `spot_termination_queue_url` (String) The URL of an SQS queue for enhanced spot termination handling. When set, enables Enhanced mode using AWS Node Termination Handler to gracefully drain nodes within the 2-minute spot interruption window. When omitted, Simple mode is used (reactive replacement via MachineHealthCheck). The queue must be an SQS queue in the same region as the cluster.
...
- `upgrade_acknowledgements_for` (String) Indicates acknowledgment of agreements required to upgrade the cluster version between minor versions (e.g. a value of "4.12" indicates acknowledgment of any agreements required to upgrade to OpenShift 4.12.z from 4.11 or before).
...
- `mode` (String) AutoNode mode. Currently only `enabled` is supported.
- `role_arn` (String) AWS IAM role ARN used by the Karpenter controller when AutoNode is enabled.
