...
This Terraform example creates a Windows License-Included **machine pool** for an existing ROSA HCP cluster. Windows LI MachinePools support running Windows Virtualization workloads with License compliance on ROSA HCP clusters.
...
* You have already created a ROSA HCP OpenShift Cluster Manager cluster.
...
1. To run the `terraform apply` you need to set up some variables. This guide uses environmental variables. For more on Terraform variables, see [Managing Variables](https://developer.hashicorp.com/terraform/enterprise/workspaces/variables/managing-variables) in the Terraform documentation.
    ...
    * Machine pool name:
        ...
        ```
    * AWS instance type. For Windows LI support, this must be a baremetal instance:
        ```
...
1. In your command-line interface, run the following command to verify that the machine pools are created:
    ...
    ````
2. To verify that the MachinePool indeed is set up with Windows Licence Included, you can describe the MachinePool:
    ````
    rosa describe machinepool -c <cluster_id> <machinepool_name>
    ````
    `Image Type` should be `Windows`
## Resource clean up
