...
type ClusterRosaHCPResourceType struct {
}
...
func (t *ClusterRosaHCPResourceType) GetSchema(ctx context.Context) (result tfsdk.Schema,
...
func (t *ClusterRosaHCPResourceType) NewResource(ctx context.Context,
...
func createHCPClusterObject(ctx context.Context,
	...
	*/
	...
	builder, err = buildHCPProxy(state, builder)
	if err != nil {
...
func buildHCPProxy(state *ClusterRosaHCPState, builder *cmv1.ClusterBuilder) (*cmv1.ClusterBuilder, error) {
	proxy := cmv1.NewProxy()
	if state.Proxy != nil {
		httpsProxy := ""
		httpProxy := ""
...
func createAdminCredentials(ctx context.Context, response *tfsdk.CreateResourceResponse,
	...
	if err != nil {
		response.Diagnostics.AddWarning(
			"Can't poll state of cluster",
			fmt.Sprintf(
				"Can't poll state of cluster '%s': %v",
				state.ID.Value, err,
			),
		)
		return
	}
	tflog.Debug(ctx, fmt.Sprintf("Cluster '%s' is Ready", state.ID.Value))

	// Create the identity provider:
	idpBuilder := cmv1.NewIdentityProvider()
	idpBuilder.Name(state.AdminCredentials.Username.Value)
	...
	htpasswdIDP := cmv1.NewHTPasswdIdentityProvider().Users(cmv1.NewHTPasswdUserList().Items(
		cmv1.NewHTPasswdUser().Username(state.AdminCredentials.Username.Value).Password(state.AdminCredentials.Password.Value),
	))
	...
	if err != nil {
		response.Diagnostics.AddWarning(
			...
			fmt.Sprintf(
				...
				state.AdminCredentials.Username.Value, state.ID.Value, err,
			),
		)
		return
	}
	tflog.Debug(ctx, fmt.Sprintf("Created identity provider '%s' for cluster '%s'", idp.Body().ID(), state.ID.Value))

	// Create the membership:
	userBuilder := cmv1.NewUser()
	userBuilder.ID(state.AdminCredentials.Username.Value)
	...
	if err != nil {
		response.Diagnostics.AddWarning(
			...
			fmt.Sprintf(
				...
				"cluster-admins", state.ID.Value, err,
			),
		)
		return
	}
	tflog.Debug(ctx, fmt.Sprintf("Created group membership '%s' for cluster '%s'", group.Body().ID(), state.ID.Value))
}

func (r *ClusterRosaHCPResource) Create(ctx context.Context,
	...
	// Save the state:
	err = populateRosaHCPClusterState(ctx, object, state, DefaultHttpClient{})
	if err != nil {
		response.Diagnostics.AddError(
	...
	if state.AdminCredentials != nil {
		...
		createAdminCredentials(ctx, response, state, resource)
	}
...
func (r *ClusterRosaHCPResource) Read(ctx context.Context, request tfsdk.ReadResourceRequest,
...
func (r *ClusterRosaHCPResource) Update(ctx context.Context, request tfsdk.UpdateResourceRequest,
	...
	clusterBuilder, _, err := updateHCPNodes(state, plan, clusterBuilder)
	if err != nil {
		response.Diagnostics.AddError(
	...
	clusterBuilder, _, err = updateHCPProxy(state, plan, clusterBuilder)
	if err != nil {
		response.Diagnostics.AddError(
			"Can't update cluster",
			fmt.Sprintf(
	...
	shouldPatchProperties := shouldPatchHCPProperties(state, plan)
	...
	// Update the state:
	err = populateRosaHCPClusterState(ctx, object, plan, DefaultHttpClient{})
	if err != nil {
		response.Diagnostics.AddError(
			"Can't populate cluster state",
			fmt.Sprintf(
...
func updateHCPProxy(state, plan *ClusterRosaHCPState, clusterBuilder *cmv1.ClusterBuilder) (*cmv1.ClusterBuilder, bool, error) {
	...
	if shouldUpdateProxy {
		var err error
		clusterBuilder, err = buildHCPProxy(plan, clusterBuilder)
		if err != nil {
...
func updateHCPNodes(state, plan *ClusterRosaHCPState, clusterBuilder *cmv1.ClusterBuilder) (*cmv1.ClusterBuilder, bool, error) {
...
func (r *ClusterRosaHCPResource) Delete(ctx context.Context, request tfsdk.DeleteResourceRequest,
...
func (r *ClusterRosaHCPResource) ImportState(ctx context.Context, request tfsdk.ImportResourceStateRequest,
...
func populateRosaHCPClusterState(ctx context.Context, object *cmv1.Cluster, state *ClusterRosaHCPState, httpClient HttpClient) error {
	...
	state.HypershiftEnabled = types.Bool{
		Value: object.Hypershift().Enabled(),
	}
...
func shouldPatchHCPProperties(state, plan *ClusterRosaHCPState) bool {
