...
func fetchNotificationContactsWithIDs(
	...
	for {
		...
		fetched += len(listResp.Items)
		if fetched >= listResp.Total {
			break
		}
		if len(listResp.Items) == 0 {
			return nil, fmt.Errorf(
				"can't read notification contacts for subscription '%s': "+
					"server returned empty page %d but only %d of %d contacts fetched",
				subscriptionID, page, fetched, listResp.Total)
		}
		page++
	}
	return result, nil
...
// POST to the notification_contacts sub-resource. On success it returns the
// canonical username from the API response. On 409 (contact already exists)
// it re-fetches contacts to resolve the canonical username.
func addNotificationContact(
	...
	if resp.Status() == http.StatusConflict {
		contacts, fetchErr := FetchNotificationContacts(ctx, connection, subscriptionID)
		if fetchErr != nil {
			return "", fmt.Errorf(
				"can't resolve canonical username for '%s' after 409 conflict: %w",
				username, fetchErr,
			)
		}
		var unseen []string
		...
		default:
			return "", fmt.Errorf(
				"can't resolve canonical username for '%s' after 409 conflict: "+
					"multiple unresolved contacts prevent safe identification",
				username,
			)
		}
	}
	...
	var account notificationContactResponse
	if err := json.Unmarshal(resp.Bytes(), &account); err == nil && account.Username != "" {
...
// The returned bool indicates whether any mutation (POST or DELETE) was attempted.
func UpdateNotificationContacts(
	...
	for _, username := range deduplicated {
		if _, exists := current[username]; !exists {
			canonical, err := addNotificationContact(ctx, connection, subscriptionID, username, seen)
			if err != nil {
	...
	mutated := len(added) > 0 || len(addErrors) > 0
	...
	if len(addErrors) > 0 {
		...
		return mutated, fmt.Errorf("%s", summary)
	}
