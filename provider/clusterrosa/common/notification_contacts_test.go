...
var _ = Describe("Notification contacts helpers", func() {
	Context("NotificationContactsResourceSchema", func() {
		var validators []validator.Set

		BeforeEach(func() {
			schema := NotificationContactsResourceSchema()
			validators = schema.Validators
		})

		validateSet := func(values ...string) validator.SetResponse {
			elements := make([]attr.Value, len(values))
			for i, v := range values {
				elements[i] = types.StringValue(v)
			}
			setVal := types.SetValueMust(types.StringType, elements)
			req := validator.SetRequest{
				Path:           path.Root("notification_contacts"),
				PathExpression: path.MatchRoot("notification_contacts"),
				ConfigValue:    setVal,
			}
		...
		It("accepts valid usernames", func() {
			resp := validateSet("alice", "bob_smith")
			Expect(resp.Diagnostics.HasError()).To(BeFalse())
		})

		It("accepts email-style usernames without whitespace", func() {
			resp := validateSet("alice@example.com")
			Expect(resp.Diagnostics.HasError()).To(BeFalse())
		})

		It("rejects usernames with leading whitespace", func() {
			resp := validateSet(" alice")
			Expect(resp.Diagnostics.HasError()).To(BeTrue())
		})

		It("rejects usernames with trailing whitespace", func() {
			resp := validateSet("alice ")
			Expect(resp.Diagnostics.HasError()).To(BeTrue())
		})

		It("rejects usernames with internal whitespace", func() {
			resp := validateSet("alice smith")
			Expect(resp.Diagnostics.HasError()).To(BeTrue())
		})
		...
		It("has UseStateForUnknown plan modifier", func() {
			schema := NotificationContactsResourceSchema()
			Expect(schema.PlanModifiers).NotTo(BeEmpty())
		})
	})
	...
	Context("API operations", func() {
		...
		Context("UpdateNotificationContacts", func() {
			...
			It("continues adding remaining contacts after one fails and reports summary", func() {
				...
				err := UpdateNotificationContacts(ctx, connection, "sub-123", []string{"bad_user", "good_user"})
				Expect(err).To(HaveOccurred())
			...
			It("deduplicates contacts and sends only one POST per unique username", func() {
				...
				err := UpdateNotificationContacts(ctx, connection, "sub-123", []string{"user1", "user1", "user1"})
				Expect(err).NotTo(HaveOccurred())
			...
			It("skips removals when an addition fails", func() {
				...
				err := UpdateNotificationContacts(ctx, connection, "sub-123", []string{"bad_user"})
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("failed to add 1 of 1"))
				Expect(server.ReceivedRequests()).To(HaveLen(2))
			})

			It("reports HTTP 409 when username and email resolve to the same account", func() {
				server.AppendHandlers(
					ghttp.CombineHandlers(
						ghttp.VerifyRequest("GET", "/api/accounts_mgmt/v1/subscriptions/sub-123/notification_contacts"),
						sdktesting.RespondWithJSON(http.StatusOK, `{
				...
				err := UpdateNotificationContacts(ctx, connection, "sub-123", []string{"user1", "user1@example.com"})
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("failed to add 1 of 2"))
				Expect(err.Error()).To(ContainSubstring("user1@example.com"))
				Expect(err.Error()).To(ContainSubstring("Successfully added: user1"))
			})
