...
var _ = Describe("Rosa HCP Sts cluster", func() {
	...
	Context("populateRosaHcpClusterState", func() {
		It("Converts correctly a Cluster object into a ClusterRosaHcpState", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(populateRosaHcpClusterState(context.Background(), clusterObject, clusterState, nil)).To(Succeed())
		...
		It("Check trimming of oidc url with https perfix", func() {
			...
			err = populateRosaHcpClusterState(context.Background(), clusterObject, clusterState, nil)
			Expect(err).ToNot(HaveOccurred())
	...
	Context("Day 1 Log Forwarding", func() {
		It("Creates cluster object with log forwarders", func() {
			...
			clusterState.LogForwarders = logForwarders
		...
		It("Creates cluster object without log forwarders when not configured", func() {
			clusterState := generateBasicRosaHcpClusterState()
			clusterState.LogForwarders = types.ListNull(types.ObjectType{
				AttrTypes: map[string]attr.Type{
					"s3":           types.ObjectType{},
