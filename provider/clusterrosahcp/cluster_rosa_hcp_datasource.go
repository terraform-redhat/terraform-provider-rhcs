		...
		Attributes: map[string]schema.Attribute{
			...
			"multi_az": schema.BoolAttribute{
				Description: "Indicates if the cluster should be deployed to " +
					"multiple availability zones. Default value is 'true'.",
				Computed: true,
			},
			...
			"billing_account_id": schema.StringAttribute{
				Description: "Identifier of the billing account. " + common.ValueCannotBeChangedStringDescription,
				Computed:    true,
			},
			...
			// Those attributes were copied from cluster_rosa_hcp resource in order to use the same state struct.
			// We can't change the rosa_hcp struct to include Embedded Structs due to that issue: https://github.com/hashicorp/terraform-plugin-framework/issues/242
