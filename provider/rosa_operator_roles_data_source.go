...
func (t *RosaOperatorRolesDataSource) Read(ctx context.Context, request tfsdk.ReadDataSourceRequest,
	...
	stsOperatorRolesList.Items().Each(func(stsCredentialRequest *cmv1.STSCredentialRequest) bool {
		t.logger.Info(ctx, "************** ")
		stsOperatorMap[stsCredentialRequest.Operator().Name()] = stsCredentialRequest.Operator()
		return true
	})
		...
		for _, operatorRole := range sts.OperatorIAMRoles() {
			r := OperatorIAMRole{
				...
				PolicyName: types.String{
					Value: getRoleName(state.AccountRolePrefix.Value, operatorRole.Namespace(), operatorRole.Name()),
				},
				ServiceAccounts: getServiceAccount(stsOperatorMap[operatorRole.Name()].ServiceAccounts(), operatorRole.Namespace()),
			}
