...
const versionListPage1 = `{
	...
	"items": [{
			...
			"raw_id": "4.10.0",
			"available_channels": ["stable-4.10", "eus-4.10", "fast-4.10", "stable-4.11", "fast-4.11"]
		},
		{
			"kind": "Version",
			...
			"raw_id": "4.10.1",
			"available_channels": ["stable-4.10", "eus-4.10", "fast-4.10", "stable-4.11", "fast-4.11", "stable-4.12", "fast-4.12"]
		},
		{
			"kind": "Version",
			"id": "openshift-4.14.0",
			...
			"raw_id": "4.14.0",
			"available_channels": ["stable-4.14", "fast-4.14", "stable-4.15", "fast-4.15"]
		},
		{
			"kind": "Version",
			...
			"raw_id": "4.11.1",
			"available_channels": ["stable-4.11", "fast-4.11", "stable-4.12", "fast-4.12"]
		},
		{
			"kind": "Version",
			"id": "openshift-v4.15.0",
			"href": "/api/clusters_mgmt/v1/versions/openshift-v4.15.0",
			"raw_id": "4.15.0",
			"available_channels": ["stable-4.15", "fast-4.15", "stable-4.16", "fast-4.16"]
		}
	]
}`
...
var _ = Describe("rhcs_cluster_rosa_classic - create", func() {
	...
	Context("rhcs_cluster_rosa_classic - create", func() {
		...
		Context("Test channel parameter", func() {
			...
			It("does not drift when channel_group is omitted and API returns stable", func() {
				...
				TestServer.AppendHandlers(
					...
					// Create cluster - API returns channel_group="stable"
					CombineHandlers(
						VerifyRequest(http.MethodPost, "/api/clusters_mgmt/v1/clusters"),
						RespondWithPatchedJSON(http.StatusCreated, template, `[
						{
			...
			It("Fails with incompatible channel-only config and ec2_metadata_http_tokens", func() {
				// Channel "stable-4.10" resolves to 4.10.1, which fails ec2_metadata_http_tokens validation (minimum 4.11.0)
				TestServer.AppendHandlers(
					CombineHandlers(
						VerifyRequest(http.MethodGet, "/api/clusters_mgmt/v1/versions"),
						RespondWithJSON(http.StatusOK, versionListPage1),
				...
				runOutput.VerifyErrorContainsSubstring("version '4.10.1' is not supported with ec2_metadata_http_tokens")
				runOutput.VerifyErrorContainsSubstring("minimum supported version is 4.11.0")
			})
		})
	...
	Context("Notification Contacts", func() {
		...
		It("Removes a notification contact", func() {
			...
			// Update - remove bob, keep only alice
			TestServer.AppendHandlers(
