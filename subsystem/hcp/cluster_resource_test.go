		...
		Context("Test channel parameter", func() {
			...
			It("creates cluster with both channel and version specified", func() {
				TestServer.AppendHandlers(
					...
					CombineHandlers(
						VerifyRequest(http.MethodGet, "/api/clusters_mgmt/v1/versions"),
						VerifyFormKV("search", "enabled = 'true' AND rosa_enabled = 'true' AND raw_id = '4.14.1' AND hosted_control_plane_enabled = 'true'"),
			...
			It("fails when channel is incompatible with version", func() {
				...
				runOutput.VerifyErrorContainsSubstring("channel 'stable-4.16' is not available for version '4.14.1'")
			})
		...
		Context("Update channel parameter", func() {
			...
			It("successfully updates channel", func() {
				TestServer.AppendHandlers(
					...
					CombineHandlers(
						VerifyRequest(http.MethodGet, "/api/clusters_mgmt/v1/versions"),
						VerifyFormKV("search", "enabled = 'true' AND rosa_enabled = 'true' AND raw_id = '4.14.0' AND hosted_control_plane_enabled = 'true'"),
						RespondWithJSON(http.StatusOK, `{
							"kind": "VersionList",
							"page": 1,
							"size": 1,
