...
var _ = Describe("Image Mirror Resource", func() {
	Context("Image Mirror CRUD operations", func() {
		...
		It("Validates DNS 1123 subdomain names", func() {
			// These validation tests can run without API implementation
			TestServer.AppendHandlers(
				CombineHandlers(
					VerifyRequest(http.MethodGet, "/api/clusters_mgmt/v1/clusters/123"),
					RespondWithJSON(http.StatusOK, template),
			...
			// Test invalid registry with uppercase characters
			Terraform.Source(`
			resource "rhcs_image_mirror" "invalid_registry" {
				cluster_id = "123"
				source     = "Invalid-Registry.Com/team"  # Contains uppercase
				mirrors    = ["mirror.corp.com/team"]
			}`)
			
			runOutput := Terraform.Apply()
			Expect(runOutput.ExitCode).ToNot(BeZero())
			runOutput.VerifyErrorContainsSubstring("Invalid Registry Format")
		})

		It("Validates zero-egress protected registries", func() {
			TestServer.AppendHandlers(
				CombineHandlers(
					VerifyRequest(http.MethodGet, "/api/clusters_mgmt/v1/clusters/123"),
					RespondWithJSON(http.StatusOK, template),
			...
			// Test protected registry source
			Terraform.Source(`
			resource "rhcs_image_mirror" "protected_registry" {
				cluster_id = "123"
				source     = "quay.io/openshift-release-dev/ocp-release"  # Protected registry
				mirrors    = ["mirror.corp.com/team"]
			}`)
			
			runOutput := Terraform.Apply()
			Expect(runOutput.ExitCode).ToNot(BeZero())
			runOutput.VerifyErrorContainsSubstring("Zero-Egress Protected Registry")
		})
