...
- When omitted and the roles define ambiguous external IDs, create proceeds with a warning and no external ID is sent to OCM, matching ROSA CLI non-interactive behavior.
...
For ROSA Classic, use `rhcs_cluster_rosa_classic` and the `rosa-classic/rhcs//modules/account-iam-resources` module with the same `trust_policy_external_id` wiring.
