...
var _ = Describe("Upgrade", func() {
	...
	It("on Y-stream - [id:63152] [id:72475]", ci.Upgrade,
		func() {
			...
			_, err = clusterService.Apply(clusterArgs)
			if err != nil && !profile.GetClusterType().HCP {
				// Known issue: classic Y-stream upgrades can fail on first apply
				// due to upgrade_acknowledgements_for not being honored. Retry once.
				Logger.Warnf("Classic Y-stream upgrade failed on first apply (known issue), retrying: %v", err)
				_, err = clusterService.Apply(clusterArgs)
			}
			Expect(err).ToNot(HaveOccurred())
