...
var _ = Describe("HCP ImageMirror", ci.Day2, func() {
	...
	It("can be created with required attributes - [id:76001]",
		ci.Critical, func() {
	...
	It("can be updated to add and remove mirrors - [id:76002]",
		ci.Critical, func() {
	...
	It("can validate invalid configurations - [id:76003]",
		ci.Medium, func() {
	...
	It("enforces immutable fields - [id:76004]",
		ci.Medium, func() {
			By("Create image mirror")
			...
			By("Try to update type (should trigger replace)")
			// Note: Currently only 'digest' type is supported, so this test
			// would be more relevant when additional types are added
			Logger.Info("Type field immutability test skipped - only 'digest' type currently supported")
		})

	It("can handle multiple image mirrors - [id:76005]",
		ci.High, func() {
