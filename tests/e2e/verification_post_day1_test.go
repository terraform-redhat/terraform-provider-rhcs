...
var _ = Describe("Verify cluster", func() {
	...
	It("fips is correctly enabled/disabled - [id:63140]", ci.Day1Post, ci.High, func() {
		Expect(cluster.FIPS()).To(Equal(profileHandler.Profile().IsFIPS()))
	})

	It("private_link is correctly enabled/disabled - [id:63133]", ci.Day1Post, ci.High, func() {
		Expect(cluster.AWS().PrivateLink()).To(Equal(profileHandler.Profile().IsPrivateLink()))
	})

	It("etcd-encryption is correctly enabled/disabled - [id:63143]", ci.Day1Post, ci.High, func() {
		Expect(cluster.EtcdEncryption()).To(Equal(profileHandler.Profile().IsEtcd()))
	})

	It("compute_machine_type is correctly set - [id:64023]", ci.Day1Post, ci.Medium, func() {
		if profileHandler.Profile().GetComputeMachineType() == "" {
			Skip("No compute_machine_type is configured for the cluster. skipping the test.")
		}
		Expect(cluster.Nodes().ComputeMachineType().ID()).To(Equal(profileHandler.Profile().GetComputeMachineType()))
	})

	It("compute_replicas is correctly set - [id:73153]", ci.Day1Post, ci.Medium, func() {
		if profileHandler.Profile().GetComputeReplicas() <= 0 {
			Skip("No compute_replicas is configured for the cluster. skipping the test.")
		}
		Expect(cluster.Nodes().Compute()).To(Equal(profileHandler.Profile().GetComputeReplicas()))
	})

	It("availability zones and multi-az are correctly set - [id:63141]", ci.Day1Post, ci.Medium, func() {
		getResp, err := cms.RetrieveClusterDetail(cms.RHCSConnection, clusterID)
		zonesArray := strings.Split(profileHandler.Profile().GetZones(), ",")
		clusterAvailZones := getResp.Body().Nodes().AvailabilityZones()
		Expect(err).ToNot(HaveOccurred())
		Expect(getResp.Body().MultiAZ()).To(Equal(profileHandler.Profile().IsMultiAZ()))

		if profileHandler.Profile().GetZones() != "" {
			Expect(clusterAvailZones).To(Equal(helper.JoinStringWithArray(profileHandler.Profile().GetRegion(), zonesArray)))
		} else {
			// the default zone for each region
			Expect(clusterAvailZones[0]).To(Equal(fmt.Sprintf("%sa", profileHandler.Profile().GetRegion())))
		}
	})

	It("compute_labels are correctly set - [id:68423]", ci.Day1Post, ci.High, func() {
		if profileHandler.Profile().IsLabeling() {
	...
	It("AWS tags are set - [id:63777]", ci.Day1Post, ci.High, func() {
		...
		if profileHandler.Profile().IsTagging() {
	...
	It("can be imported - [id:65684]",
		ci.Day2, ci.Medium, ci.NonHCPCluster, ci.FeatureImport, func() {
			...
			Expect(output).To(ContainSubstring(profileHandler.Profile().GetRegion()))
	...
	It("worker disk size is set correctly - [id:69143]",
		ci.Day1Post, ci.Critical, ci.NonHCPCluster,
		func() {
			switch profileHandler.Profile().GetWorkerDiskSize() {
			case 0:
				Expect(cluster.Nodes().ComputeRootVolume().AWS().Size()).To(Equal(300))
			default:
				Expect(cluster.Nodes().ComputeRootVolume().AWS().Size()).To(Equal(profileHandler.Profile().GetWorkerDiskSize()))
			}
		})
	...
	It("account roles/policies unified path is correctly set - [id:63138]", ci.Day1Post, ci.Medium, func() {
		...
		Expect(err).ToNot(HaveOccurred())
		Expect(profileHandler.Profile().GetUnifiedAccRolesPath()).Should(ContainSubstring(unifiedPath))
	})

	It("customer-managed KMS key is set correctly - [id:63334]", ci.Day1Post, ci.Medium, func() {
		if !profileHandler.Profile().IsKMSKey() {
	...
	It("imdsv2 value is set correctly - [id:63950]", ci.Day1Post, ci.Critical, ci.NonHCPCluster, func() {
		if profileHandler.Profile().GetEc2MetadataHttpTokens() != "" {
			Expect(string(cluster.AWS().Ec2MetadataHttpTokens())).To(Equal(profileHandler.Profile().GetEc2MetadataHttpTokens()))
		} else {
